...
function calcCrc(uint8Array) {
	...
	for (let n = 0; n < uint8Array.length; n++) c = pngDataTable[(c ^ uint8Array[n]) & 255] ^ c >>> 8;
	return c ^ -1;
}
function searchStartOfPhys(uint8Array) {
	const length = uint8Array.length - 1;
	for (let i = length; i >= 4; i--) if (uint8Array[i - 4] === 9 && uint8Array[i - 3] === _P && uint8Array[i - 2] === _H && uint8Array[i - 1] === _Y && uint8Array[i] === _S) return i - 3;
	return 0;
}
...
function detectPhysChunkFromDataUrl(dataUrl) {
	...
	if (b64index === -1) b64index = dataUrl.indexOf(b64PhysSignature2);
	if (b64index === -1) b64index = dataUrl.indexOf(b64PhysSignature3);
	return b64index;
}
	...
	try {
		return await new Promise((resolve, reject) => {
			canvas.toBlob((blob) => {
				...
				else reject(/* @__PURE__ */ new Error("Blob is null"));
			}, type, quality);
		});
	} catch (error) {
		if (SUPPORT_ATOB) return dataUrlToBlob(canvas.toDataURL(type, quality));
		throw error;
	}
}
function dataUrlToBlob(dataUrl) {
	...
	for (let i = 0; i < length; i += 1) buffer[i] = decoded.charCodeAt(i);
...
async function waitUntilLoad(node, options) {
	if (isHTMLElementNode(node)) if (isImageElement(node) || isVideoElement(node)) await loadMedia(node, options);
...
function createCanvas(ownerDocument, context) {
	...
	if (max) {
		if (canvas.width > max || canvas.height > max) if (canvas.width > max && canvas.height > max) if (canvas.width > canvas.height) {
...
function cloneIframe(iframe, context) {
	try {
		if (iframe?.contentDocument?.body) return cloneNode(iframe.contentDocument.body, context);
	} catch (error) {
...
var pseudoClasses = ["::before", "::after"];
...
var excludeParentNodes = /* @__PURE__ */ new Set(["symbol"]);
...
async function domToBlob(node, options) {
	...
	if (["image/png", "image/jpeg"].includes(type) && dpi) {
		...
		if (type === "image/png") uint8Array = changePngDpi(uint8Array, dpi);
		else if (type === "image/jpeg") uint8Array = changeJpegDpi(uint8Array, dpi);
...
async function domToImage(node, options) {
	...
	const image = createImage(type === "image/svg+xml" ? await domToSvg(context) : await domToDataUrl(context), ownerDocument);
...
async function domToPixel(node, options) {
	const canvas = await domToCanvas(await orCreateContext(node, options));
